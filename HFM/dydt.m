function dydt = dydt(t,y,R,T,F_CH4_0,V)  %F_CH4_0 passed as F_CH4 in HFM.m
global outflow;
%R = 8.314; %J/kgK
%V = 0.5; %m^3 - unsure
...
%No need for conversion of pressure to concentration - see Xu-Froment
%Can this be assumed to be the case for ROM as well?
DEN=1+KCO*y(5)+KH2*y(4)+KCH4*y(1)+KH2O*y(2)/y(4); 
r1=((kin1/y(4)^2.5)/DEN^2)*(y(1)*y(2)-y(4)^3*y(5)/K1); %kmol/(kgcat.h) - mol/( g cat.h)
r2=((kin2/y(4))/DEN^2)*(y(5)*y(2)-y(4)*y(3)/K2); %kmol/(kgcat.h)
r3=((kin3/y(4)^3.5)/DEN^2)*(y(1)*y(2)^2-y(4)^4*y(3)/K3); %kmol/(kgcat.h)

%Conversion from (kmol/kgcat.h) to (kmol/kgcat.s)
%r1 = r1/3.600; r2 = r2/3.600; r3 = r3/3.600;

%Conversion from (kmol/kgcat.h) to mol/(gcat.h)

% Rates remain the same

%Conversion from kmol/kgcat.hr to mol/kgcat.hr
r1 = r1*1000; r2 = r2*1000; r3 = r3*1000;



F_in = F_CH4_0 + F_H2O_0 + F_H2_0 + F_CO_0 + F_CO2_0; % All in mol/h

% *3600 - conversion from per hour to per second
% /101325 - conversion from Pa to bar. 

%Conversion factor - Pa to bar - mol/hr to mol/s
factor = 1/(101325);%3600/101325
 
dydt(1) = (R*T/V)*factor*(F_CH4_0 - ((r1 + r3)*V) - (y(1)/P_total)*(F_in + 2*V*r1 + 2*V*r3));   %V as g cat - confused about this. 
